...
$form = ActiveForm::begin(['id' => 'personForm']);
...
<div class="row">
    <div class="col-sm-6 ">
        ...
        <?=Html::a('<i class="fa fa-list" ></i> จัดการข้อมูลต่างๆ ที่เกี่ยวข้อง',['/persons/default/value'],
                [
                    'target' => '_blank',
                    'class' => 'btn btn-warning btn-xs'
                ])?>
        </p>
        <p style="color:#FF0000;">* กรุณากรอกให้ครบถ้วน</p>
        <p style="color:#999999;">** หากไม่มีให้เลือก สามารถพิมพ์เพิ่มได้เลยแล้วก็กด Enter</p>
        <p>&nbsp;</p>
    </div>
</div>
...
use yii\web\View;
$js = <<<JS
    $('form#personForm').submit(function() {
        var form = $('form#personForm');
    if (form.find('.required.has-error').length>0) {
        console.log('has-error:'+form.find('.has-error').length);
        alert('* กรุณากรอกให้ครบถ้วน ในแถบอื่นๆ');
        return false;
    }
