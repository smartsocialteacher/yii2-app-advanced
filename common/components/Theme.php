...
use yii\helpers\Url;
use common\models\SysSite;
use common\models\SysTemplates;
...
class Theme extends yii\base\Component {
    ...
    public function init() {
       ...
       /*echo "<pre>";
        ...
        echo "</pre>";*/
        //echo \Yii::$app->request->web;
       // echo "<br/><br/><br/><br/>";
        //echo Url::to();
        $model = SysSite::findOne([
            'site_path'=>\Yii::$app->request->web
        ]);   
        // web default template
        $this->active= $model?$model->temp->temp_title:'web';
              

        if ($this->baseUrl === null) {
           $this->baseUrl = '@themes/' . $this->active;
        }
        $this->baseUrl = rtrim(Yii::getAlias($this->baseUrl), '/');
        if ($this->basePath === null) {
            $this->basePath = '@themes/' . $this->active;
        }
        $this->basePath = Yii::getAlias($this->basePath);
        if (empty($this->pathMap)) {
            $this->pathMap = ['@app/views' => [$this->basePath]];
        }
    }
    /**
